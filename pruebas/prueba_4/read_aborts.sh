...
for archivo in "$directorio"/*.stats; do
    if [ -f "$archivo" ]; then
        # Extraer la línea 5 del archivo actual y verificar si contiene un número mayor o igual que 1 más de 3 veces
        ...
        if [ "$num_ocurrencias" -ge 2 ]; then
            echo "Contenido de la línea 9 en $archivo: $contenido_linea_5"
        fi
    fi
done
